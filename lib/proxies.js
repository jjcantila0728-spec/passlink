...
// Webshare (or any provider) "download list" URL that returns one proxy per
// line. Supports `ip:port`, `ip:port:user:pass`. Set PASSLINK_WEBSHARE_LIST_URL
// to the provider's authenticated download link. Cached briefly so we don't
// re-download on every request.
let _wsCache = { at: 0, list: [] };
async function listUrlProxies() {
  const url = String(process.env.PASSLINK_WEBSHARE_LIST_URL || "").trim();
  if (!url) return [];
  if (Date.now() - _wsCache.at < 5 * 60 * 1000) return _wsCache.list;
  try {
    ...
    const list = text
      ...
      .map((line) => {
        ...
        return normalizeProxy(user && pass ? `http://${user}:${pass}@${ip}:${port}` : `${ip}:${port}`);
      })
      .filter(Boolean);
    _wsCache = { at: Date.now(), list };
    return list;
  } catch {
    return _wsCache.list;
  }
}
...
export async function getProxyCandidates() {
  ...
  const [listed, stored] = await Promise.all([
    listUrlProxies().catch(() => []),
    getStoredProxies().catch(() => []),
  ]);
  const seen = new Set();
  const out = [];
  // Stored first: a previously-validated proxy is promoted to the front there,
  // so it's tried before the (mostly-blocked) raw provider list.
  for (const p of [...env, ...stored, ...listed]) {
...
// Uses undici's own fetch + ProxyAgent (global fetch + this package's ProxyAgent
// are incompatible — see lib/bypass.js).
export async function probeProxy(proxy, target, { timeoutMs = 9000 } = {}) {
  ...
  try {
    const agent = new mod.ProxyAgent({ uri: proxy, connectTimeout: 4000, headersTimeout: timeoutMs, bodyTimeout: timeoutMs });
    const r = await mod.fetch(target, {
      method: "GET",
      redirect: "manual",
      dispatcher: agent,
