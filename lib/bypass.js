...
// Optional outbound proxy. Some shorteners (e.g. linkshortx.in via Hostinger's
// "hcdn") hard-block datacenter IP ranges with a 403, so a request straight from
// the serverless host never sees the link page. Point PASSLINK_PROXY_URL at a
// residential/mobile HTTP(S) proxy and the affected handlers retry the whole
// flow through it. Unset = direct only (fine for sites that don't IP-block).
const PROXY_URL = (process.env.PASSLINK_PROXY_URL || "").trim() || null;
...
export async function bypass(rawUrl, { onNote, context, skipCache = false } = {}) {
    ...
    if (handler) {
      ...
      try {
        next = await handler.resolve(current, {
          fetchWithTimeout, onNote: emit, proxyUrl: PROXY_URL, isBlockStatus,
        });
      } catch (err) {
  ...
  if (current === source && (blockedHost || handlerFailed)) {
    ...
    const hint = PROXY_URL
      ? "the proxy is also blocked or the page changed"
      : "it blocks datacenter IPs — set PASSLINK_PROXY_URL to a residential proxy";
    throw new Error(
      blockedHost
        ? `${host} refused our request (HTTP block) — ${hint}.`
        : `Couldn't extract a destination from ${host} — ${hint}.`,
    );
  }
