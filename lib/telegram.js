...
//
// Persistence of the dynamic allow-lists, in priority order:
//   Supabase (Postgres) -> Upstash/Vercel-KV -> file -> in-memory.

import fs from "node:fs/promises";
import path from "node:path";
import { bypass } from "./bypass.js";
import { logMod } from "./insights.js";
...
// Supabase (durable Postgres). Uses the secret service_role key, server-side
// only — RLS blocks the public/anon key from touching approvals.
const SUPABASE_URL = process.env.SUPABASE_URL;
const SUPABASE_KEY =
  process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_SERVICE_KEY;
const SUPABASE_TABLE = process.env.SUPABASE_KV_TABLE || "passlink_kv";
...
const KV_KEYS = {
  ...
  notes: "passlink:tg:notes", // `${groupId}:${name}` -> JSON({id, name, text, addedBy})
  filters: "passlink:tg:filters", // `${groupId}:${trigger}` -> JSON({id, trigger, reply})
};
...
// Supabase Postgres store. Single table `passlink_kv` (coll, id, data jsonb).
// Uses the secret service_role key so RLS is bypassed server-side. PostgREST
// gives us upsert (Prefer: resolution=merge-duplicates) and filtered delete.
function supabaseStore() {
  ...
  return {
    name: "supabase",
    async list(coll) {
      const res = await req(`${base}?select=data&coll=${eq(coll)}`);
      const rows = await res.json();
      return Array.isArray(rows) ? rows.map((r) => r.data).filter(Boolean) : [];
    },
    async get(coll, id) {
      const res = await req(`${base}?select=data&coll=${eq(coll)}&id=${eq(id)}&limit=1`);
      const rows = await res.json();
      return rows?.[0]?.data ?? null;
    },
    async has(coll, id) {
      const res = await req(`${base}?select=id&coll=${eq(coll)}&id=${eq(id)}&limit=1`);
      const rows = await res.json();
    ...
    async add(coll, entry) {
      await req(base, {
        ...
        body: JSON.stringify({ coll, id: String(entry.id), data: entry }),
      });
    },
    async remove(coll, id) {
      const res = await req(`${base}?coll=${eq(coll)}&id=${eq(id)}`, {
        method: "DELETE",
  ...
  async function load() {
    ...
    try {
      ...
      cache = {
        ...
        notes: new Map(Object.entries(data.notes || {})),
        filters: new Map(Object.entries(data.filters || {})),
      };
    } catch (e) {
      if (e.code !== "ENOENT") console.warn("[passlink-tg] store read error:", e.message);
      cache = { users: new Map(), groups: new Map(), warns: new Map(), notes: new Map(), filters: new Map() };
    }
    return cache;
  }
  async function save() {
    ...
    const obj = {
      ...
      notes: Object.fromEntries(c.notes),
      filters: Object.fromEntries(c.filters),
    };
...
function pickStore() {
  if (SUPABASE_URL && SUPABASE_KEY) return supabaseStore();
...
// On Vercel, /tmp is per-instance and wiped on cold start — group approvals and
// the allowlist won't survive. Warn loudly at boot so this can't silently
// regress; the fix is to connect Vercel KV (injects KV_REST_API_URL/TOKEN),
// which makes pickStore() select the durable kvStore() automatically.
const DURABLE_STORES = new Set(["supabase", "upstash-kv"]);
if (process.env.VERCEL && !DURABLE_STORES.has(store.name)) {
  console.warn(
    "[passlink-tg] WARNING: persistent storage is NOT configured. Running on " +
      "Vercel with ephemeral /tmp — approvals and the allowlist will be lost on " +
      "cold starts. Set SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY (or connect " +
      "Vercel KV) to make them durable.",
  );
}
...
// Tell users an unapproved group can't be used, instead of staying silent.
// Passive link detection is throttled (once per group / 5 min) so a busy chat
// isn't spammed; explicit commands always get a reply.
const unapprovedNotice = new Map(); // chatId -> last-notified ms
async function notifyUnapprovedGroup(chatId, replyTo, { throttle = false } = {}) {
  if (throttle) {
    const now = Date.now();
    if (now - (unapprovedNotice.get(String(chatId)) || 0) < 5 * 60 * 1000) return;
    unapprovedNotice.set(String(chatId), now);
  }
  await sendMessage(
    chatId,
    "⛔ Unauthorized: this group isn't approved for PassLink.\nA group admin must run /approvegroup here to enable it.",
    { replyTo },
  );
}
...
const GROUP_HELP = [
  ...
  "",
  "Members: /whoami · /rules · /report (reply) · /kickme",
  "",
  "Moderator commands (group admins):",
  "/ban /unban /tban <time> /kick /mute [min] /tmute <time> /unmute",
  "/warn [reason] /unwarn /warns /promote /demote",
  "/pin /unpin /del /purge (reply) /setwelcome <text> /welcome /setrules <text>",
  "",
  "Owner only: /approvegroup /revokegroup",
  "",
  "Time format: 30m · 2h · 3d · 1w",
].join("\n");
...
async function handlePrivate(message) {
  ...
  await runBypass(chatId, url, message.message_id, { actorType: "telegram-dm", actorId: fromId });
}
...
async function handleGroup(message) {
  ...
  if (!parsed) {
    if (!(await isGroupApproved(chatId))) {
      // A bare link is a clear attempt to use the bot — tell them it's
      // unauthorized (throttled) rather than appearing dead. Other chatter stays
      // silent so the bot never spams a group it shouldn't be talking in.
      if (extractUrl(message.text)) await notifyUnapprovedGroup(chatId, message.message_id, { throttle: true });
      return;
    }
  ...
  if (cmd === "approvegroup") {
    // Owner-only: only the PassLink owner (env admin) may approve a group — NOT
    // the group's own Telegram admins.
    if (!isAdmin(fromId)) {
      return void (await sendMessage(chatId, "⛔ Only the PassLink owner can approve this group.", { replyTo: message.message_id }));
    }
    if (await isGroupApproved(chatId)) {
    ...
    await logMod({ groupId: chatId, actorId: fromId, action: "approvegroup", ok: true });
    await sendMessage(chatId, "✅ Group approved! Members can now use /bypass <link>.", { replyTo: message.message_id });
    return;
  }
  if (cmd === "revokegroup") {
    // Owner-only: only the PassLink owner (env admin) may revoke approval.
    if (!isAdmin(fromId)) {
      return void (await sendMessage(chatId, "⛔ Only the PassLink owner can revoke this group.", { replyTo: message.message_id }));
    }
    // Env pre-approved groups can't be revoked at runtime — they're owner-set.
    if (ENV_GROUP_IDS.has(String(chatId))) {
      return void (await sendMessage(chatId, "⛔ This group is approved via TELEGRAM_APPROVED_GROUP_IDS. Remove it there to revoke.", { replyTo: message.message_id }));
    }
    const had = await groups.remove(chatId);
    await logMod({ groupId: chatId, actorId: fromId, action: "revokegroup", ok: had });
    await sendMessage(chatId, had ? "✅ Group approval revoked. /bypass is disabled here." : "This group wasn't approved.", { replyTo: message.message_id });
    return;
  }

  const approved = await isGroupApproved(chatId);

  // In unapproved groups, stay quiet except to tell users it's unauthorized when
  // they explicitly try to use the bot (always reply to explicit commands).
  if (!approved) {
    if (cmd === "help" || cmd === "start" || cmd === "bypass" || cmd === "pl" || cmd === "p" || cmd === "passlink") {
      await notifyUnapprovedGroup(chatId, message.message_id);
    }
    return;
  }
  ...
  switch (cmd) {
    ...
    case "kickme": {
      if (await isGroupModerator(chatId, fromId)) {
        await sendMessage(chatId, "You're an admin — I won't kick you. 😅", { replyTo: message.message_id });
        return;
      }
      const b = await tg("banChatMember", { chat_id: chatId, user_id: fromId });
      if (b.ok) await tg("unbanChatMember", { chat_id: chatId, user_id: fromId, only_if_banned: true });
      await sendMessage(
        chatId,
        b.ok ? "👋 Done — you've been removed. You can rejoin anytime." : `❌ ${b.description || "couldn't kick you (am I an admin?)"}`,
        { replyTo: message.message_id },
      );
      return;
    }
    ...
    case "admins": {
      ...
      const admins = (r?.result || []).filter((a) => !a.user?.is_bot);
      const mentions = admins
        .map((a) => (a.user?.username ? `@${a.user.username}` : displayName(a.user)))
        .join(" ");
      await sendMessage(
        chatId,
        `⚠️ ${displayName(message.from)} reported a message to the admins.\n${mentions}`,
        { replyTo: message.reply_to_message?.message_id || message.message_id },
      );
      return;
    }
    case "bypass":
    ...
    case "passlink": {
      ...
      await runBypass(chatId, url, message.message_id, { actorType: "telegram-group", actorId: fromId, chatId });
      return;
    }
  }
  ...
  const MOD_CMDS = new Set([
    "ban", "unban", "tban", "kick", "mute", "tmute", "unmute", "warn", "unwarn", "warns",
    "promote", "demote", "pin", "unpin", "del", "delete", "purge",
    "setwelcome", "welcome", "setrules",
  ]);
  if (MOD_CMDS.has(cmd)) {
    if (!(await isGroupModerator(chatId, fromId))) {
...
// Parse a Rose-style duration ("30m", "2h", "3d", "1w") at the start of the
// remaining args (after any leading user-ID token when not replying) and return
// { seconds, reason }. seconds is null when no duration is present.
function durationFrom(message, args) {
  let rest = args.trim();
  if (!message.reply_to_message?.from) rest = rest.replace(/^-?\d+\s*/, "");
  const m = rest.match(/^(\d+)\s*([mhdw])\b\s*/i);
  if (!m) return { seconds: null, reason: rest.trim() };
  const mult = { m: 60, h: 3600, d: 86400, w: 604800 }[m[2].toLowerCase()];
  return { seconds: Number(m[1]) * mult, reason: rest.slice(m[0].length).trim() };
}

// Pretty-print a duration in seconds back to Rose-style shorthand.
...
async function handleModeration(cmd, args, message) {
  ...
  switch (cmd) {
    ...
    case "purge": {
      if (!message.reply_to_message) {
        return void (await sendMessage(chatId, "Reply to the first message you want to delete, with /purge.", { replyTo }));
      }
      const from = message.reply_to_message.message_id;
      const to = message.message_id;
      const MAX = 400; // bounded so we stay within the serverless time limit
      if (to - from > MAX) {
        return void (await sendMessage(chatId, `That range is too large (>${MAX} messages). Reply closer to where you want to start.`, { replyTo }));
      }
      const ids = [];
      for (let id = from; id <= to; id++) ids.push(id);
      let deleted = 0;
      // Telegram bulk-deletes up to 100 ids at a time; if a batch is rejected
      // (some ids aren't deletable) fall back to deleting them one by one.
      for (let i = 0; i < ids.length; i += 100) {
        const chunk = ids.slice(i, i + 100);
        const r = await tg("deleteMessages", { chat_id: chatId, message_ids: chunk });
        if (r.ok) { deleted += chunk.length; continue; }
        for (const id of chunk) {
          const d = await deleteMessage(chatId, id);
          if (d.ok) deleted++;
        }
      }
      const note = await sendMessage(chatId, `🧹 Purged ${deleted} message${deleted === 1 ? "" : "s"}.`);
      const noteId = note?.result?.message_id;
      if (noteId) { await sleep(3000); await deleteMessage(chatId, noteId).catch(() => {}); }
      return;
    }
    case "promote": {
      const t = resolveTarget(message, args);
      if (!t) return void (await sendMessage(chatId, "Point at a user: reply to their message, or pass their numeric user ID.", { replyTo }));
      const r = await tg("promoteChatMember", {
      ...
      if (r.ok) {
        // Anything after the target becomes an optional custom admin title.
        const title = (message.reply_to_message ? args : reasonFrom(args)).trim().slice(0, 16);
        if (title) await tg("setChatAdministratorCustomTitle", { chat_id: chatId, user_id: t.id, custom_title: title }).catch(() => {});
      }
      await logMod({ groupId: chatId, actorId: message.from?.id, targetId: t.id, action: "promote", ok: r.ok });
      await sendMessage(chatId, r.ok ? `⬆️ Promoted ${t.name}.` : `❌ ${r.description || "missing admin rights? (I need 'add admins')"}`, { replyTo });
      return;
    }
    case "demote": {
      const t = resolveTarget(message, args);
      ...
      await logMod({ groupId: chatId, actorId: message.from?.id, targetId: t.id, action: "demote", ok: r.ok });
      await sendMessage(chatId, r.ok ? `⬇️ Demoted ${t.name}.` : `❌ ${r.description || "can't demote — I can only demote admins I promoted."}`, { replyTo });
      return;
    }
  }
  ...
  // Audit every target-acting moderation command (best-effort).
  if (cmd !== "warns") {
    await logMod({
      groupId: chatId, actorId: message.from?.id, targetId: target.id,
      action: cmd, reason: reasonFrom(args) || null,
    });
  }
  ...
  switch (cmd) {
    ...
    case "tban": {
      const { seconds, reason } = durationFrom(message, args);
      const payload = { chat_id: chatId, user_id: target.id };
      if (seconds) payload.until_date = Math.floor(Date.now() / 1000) + seconds;
      const r = await tg("banChatMember", payload);
      const dur = seconds ? ` for ${humanDur(seconds)}` : "";
      await sendMessage(chatId, r.ok ? `🔨 Banned ${target.name}${dur}${reason ? `: ${reason}` : ""}.` : `❌ ${r.description || "missing admin rights?"}`, { replyTo });
      return;
    }
    case "tmute": {
      const { seconds, reason } = durationFrom(message, args);
      ...
      const r = await tg("restrictChatMember", payload);
      const dur = seconds ? ` for ${humanDur(seconds)}` : "";
      await sendMessage(chatId, r.ok ? `🔇 Muted ${target.name}${dur}${reason ? `: ${reason}` : ""}.` : `❌ ${r.description || "missing admin rights?"}`, { replyTo });
      return;
    }
    case "warn": {
...
async function runBypass(chatId, url, userMessageId, context) {
  ...
  const settled = bypass(url, { context }).then(
