...
export const handlers = [
...

  {
    ...
    async resolve(url, { fetchWithTimeout, onNote, proxyUrl, isBlockStatus }) {
      const log = (m) => typeof onNote === "function" && onNote(`  [urlshortx] ${m}`);
      const blocked = (s) => (isBlockStatus ? isBlockStatus(s) : s === 403 || s === 429 || s === 503);

      // Try direct first. If the site IP-blocks our host (403/429/503) and a
      // residential proxy is configured, run the whole stateful flow again
      // through the proxy — cookies/CSRF must all originate from one IP, so a
      // single-request relay won't do. Longer total time is acceptable.
      let result = await attempt(null);
      if (result === BLOCKED && proxyUrl) {
        log("direct request was IP-blocked; retrying entire flow via proxy");
        result = await attempt(proxyUrl);
      }
      if (result === BLOCKED) {
