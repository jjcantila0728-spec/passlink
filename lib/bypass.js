...
// Proxied requests MUST use undici's own fetch (imported here), not Node's
// global fetch. The global fetch is backed by a *different* (Node-bundled)
// undici build, and feeding it a ProxyAgent from this package throws
// "invalid onRequestStart method". Importing both fetch + ProxyAgent from the
// same package keeps the dispatcher interface in sync. Imported on demand so
// the direct path never loads it.
let undiciMod;
...
async function proxyDispatcher(uri) {
  ...
  if (!agent) {
    agent = new ProxyAgent({ uri, connectTimeout: 10000 });
...
async function fetchWithTimeout(url, opts = {}) {
  ...
  try {
    ...
    if (proxy) {
      ...
      init.dispatcher = await proxyDispatcher(proxy);
